                    ...
                    if (selectedIndex == null) {
                      ...
                      Provider.of<CartController>(context, listen: false).addToCartAPI(widget.cart, context,
                          ...
                          shippingMethodId: widget.shippingMethodList![selectedIndex!].id).then((value) {
                            if(splashProvider.configModel?.customerVerification?.phone == 1 &&
                                profile.userInfoModel?.isPhoneVerified ==
                                    0){
                              showCustomSnackBar(getTranslated('confirmPhone', context), context);
                              Navigator.push(context, MaterialPageRoute(builder: (_) => const ProfileScreen1()));
                            }
                        else if(value.response!.statusCode == 200 && value.response?.data['status'] == 1){
